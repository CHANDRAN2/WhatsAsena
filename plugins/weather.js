...
Asena.addCommand({pattern: 'weather ?(.*)',fromMe: false, desc: Lang.WEATHER_DESC, usage: '.weather Kadavanthara'}, async (message, match) => {
	...
	try {
		...
		if (response.statusCode === 200) return await message.client.sendMessage(message.jid, '*🗺️ ' + Lang.LOCATION +':* ```' + match[1] + '```\n\n' +
		'*🌡️ ' + Lang.TEMP +':* ```' + json.main.temp_max + '°```\n' + 
		'*ℹ ' + Lang.DESC +':* ```' + json.weather[0].description + '```\n' +
		'*🌫️ ' + Lang.HUMI +':* ```%' + json.main.humidity + '```\n' + 
